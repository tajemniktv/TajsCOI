...
namespace TajsCOI.Profiler
{
    internal static class ProfilerSettingsCatalog
    {
        ...
        {
            SettingDescriptor.Float(ModId, ModDisplayName, FrameMillisecondsKey,
                "Frame spike threshold", "Absolute frame/update trigger in milliseconds.",
                50, 1, 1000, 1, "Profiler", flags: Flags),
            SettingDescriptor.Float(ModId, ModDisplayName, WaitMillisecondsKey,
                "Wait-for-sim threshold", "Absolute main-thread wait-for-simulation trigger in milliseconds.",
                20, 1, 1000, 1, "Profiler", flags: Flags),
            SettingDescriptor.Float(ModId, ModDisplayName, SimulationMillisecondsKey,
                "Simulation threshold", "Absolute simulation worker-cycle trigger in milliseconds.",
                50, 1, 1000, 1, "Profiler", flags: Flags),
            SettingDescriptor.Float(ModId, ModDisplayName, MajorPhaseMillisecondsKey,
                "Major phase threshold", "Absolute sync/render/simulation phase trigger in milliseconds.",
                25, 1, 1000, 1, "Profiler", flags: Flags),
            SettingDescriptor.Float(ModId, ModDisplayName, RelativeMultiplierKey,
                "Relative spike multiplier", "Trigger when a frame is this multiple of the rolling median.",
                3, 1.1, 20, 0.1, "Profiler", flags: Flags),
            SettingDescriptor.Float(ModId, ModDisplayName, CooldownSecondsKey,
                "Automatic capture cooldown", "Minimum seconds between automatic broad captures.",
                30, 1, 600, 1, "Profiler", flags: Flags),
            SettingDescriptor.Integer(ModId, ModDisplayName, MaximumCapturesKey,
                "Maximum automatic captures", "Bound on retained automatic spike captures per session.",
                8, 1, 64, 1, "Profiler", flags: Flags),
            SettingDescriptor.Float(ModId, ModDisplayName, PreWindowSecondsKey,
                "Spike pre-window", "Seconds of broad history retained before a trigger.",
                3, 0, 30, 1, "Profiler", flags: Flags),
            SettingDescriptor.Float(ModId, ModDisplayName, PostWindowSecondsKey,
                "Spike post-window", "Seconds of broad history retained after a trigger.",
                4, 1, 30, 1, "Profiler", flags: Flags),
